...
void ofApp::draw() {
	...
	for (int v = 0; v <= 360; v += v_span) {
		...
		int u_start = ofMap(ofNoise(noise_seed, cos(v * DEG_TO_RAD) * 0.25, sin(v * DEG_TO_RAD) * 0.25, ofGetFrameNum() * 0.015), 0, 1, -360, 360);
		int next_u = ofMap(ofNoise(noise_seed, cos((v + v_span * 0.9) * DEG_TO_RAD) * 0.25, sin((v + v_span * 0.9) * DEG_TO_RAD) * 0.25, ofGetFrameNum() * 0.015), 0, 1, -360, 360);
		int r = ofMap(ofNoise(glm::vec4(this->make_point(R, 30, u_start, v) * 0.01, ofGetFrameNum() * 0.05)), 0, 1, 0, 150);
